...
function applyOpenAICodexFallbacks(models: ModelCatalogEntry[]): void {
  const gpt53 = findOpenAICodexModel(models, OPENAI_CODEX_GPT53_MODEL_ID);
  if (gpt53 && !findOpenAICodexModel(models, OPENAI_CODEX_GPT54_MODEL_ID)) {
    models.push({
      ...gpt53,
  ...
  const baseModel = findOpenAICodexModel(models, OPENAI_CODEX_GPT53_MODEL_ID);
  if (!baseModel) {
    return;
  }
